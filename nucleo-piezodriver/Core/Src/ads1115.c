...
HAL_StatusTypeDef ADS1115_Init(I2C_HandleTypeDef *handler, uint16_t setDataRate, uint16_t setPGA) {
	...
	case ADS1115_PGA_TWOTHIRDS:
		ADS1115_voltCoef = 0.1875; // +-6.144 V
		break;

	case ADS1115_PGA_ONE:
		ADS1115_voltCoef = 0.125; // +-4.096 V
		break;

	case ADS1115_PGA_TWO:
		ADS1115_voltCoef = 0.0625; // +-2.048 V
		break;

	case ADS1115_PGA_FOUR:
		ADS1115_voltCoef = 0.03125; // +-1.024 V
		break;
	...
	int ret = HAL_I2C_IsDeviceReady(&ADS1115_I2C_Handler, (uint16_t) (ADS1115_devAddress << 1), 5, ADS1115_TIMEOUT);
	if (ret == HAL_OK) {
		return HAL_OK;
	} else {
		return HAL_ERROR;
